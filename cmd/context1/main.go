...
func findUser(rw http.ResponseWriter, r *http.Request) {
	...
	case u := <-userChan:
		...
		log.Println("Sent user response with StatusOK")
	...
	case <-ctx.Done():
		...
		log.Println("Sent user response with StatusRequestTimeout")
